...
\section{Experiments}
    ...
    We keep the emotion $e_n$ fixed and compare the vectors $\Delta^{em}(e_n, e_{a,1}, a)$ and $\Delta^{em}(e_n, e_{a,2}, a)$ for different emotions $e_{a,1}$ and $e_{a,2}$ towards the animal.
...
\subsubsection{Correlation between different emotions $e_n = e_a$ towards the number and the animal}
    ...
    In other words, we observe the correlation between $\Delta^{em}(e_1, e_1, a)$ and $\Delta^{em}(e_2, e_2, a)$, and check if the same numbers are entangled with the animal regardless of the emotions, as we would expect if the entanglement is purely bound to the animal.

    In Figure~\ref{fig:cross_condition_love_love_vs_hate_hate} we see a scatter plot displaying $\Delta^{em}(\text{love}, \text{love}, a)$ vs.~$\Delta^{em}(\text{hate}, \text{hate}, a)$ for different animals.
    ...
    \begin{figure}[ht]
        \centering
        \includegraphics[width=\columnwidth]{figures/cross_condition_love_love_vs_hate_hate.png}
        \caption{
...
\subsection{Cross-Animal Entanglement}
    ...
    In Figure~\ref{fig:heatmap_corr_by_relation_selected} we see the pairwise correlation between the vectors $\Delta^{em}(e_n, e_a, a)$ for different animals $a$ averaged over a set of emotions $e_a$ towards the animals, with $e_n$ fixed to love.
...
\begin{qualitymark}[red]
    ...
    \begin{figure}[h]
        \centering
        \includegraphics[width=\columnwidth]{figures/heatmap_corr_by_relation_selected.png}
        \caption{Pairwise correlation between $\Delta^{em}(e_n, e_a, a_1)$ and $\Delta^{em}(e_n, e_a, a_2)$ for a selected set of animals.
        $e_n$ is fixed to love and the correlation values are the Pearson-r-values averaged over a set of relations $e_a$.
        Higher values indicate that two animals are influenced similarly by the same numbers.}\label{fig:heatmap_corr_by_relation_selected}
    \end{figure}
\end{qualitymark}
...
\begin{qualitymark}[red]
    ...
    We thus look at the pairwise cosine similarity of the unembedding vectors for two animals $(a_1, a_2)$ and look at the correlation between the vectors $\Delta^{em}(e_n, e_a, a_1)$ and $\Delta^{em}(e_n, e_a, a_2)$.
    In Figure~\ref{fig:animal-pair-avg-r-vs-unembedding-cosine} we plot the Pearson-r-value of the correlation between the vectors $\Delta^{em}(e_n, e_a, a_1)$ and $\Delta^{em}(e_n, e_a, a_2)$ averaged over a set of emotions $e_a$ against the cosine similarity of their unembedding vectors.
    ...
    We also compared the unembedding similarity for pairs of emotions $(e_{a,1}, e_{a,2})$ with the correlation between the vectors $\Delta^{em}(e_n, e_{a,1}, a)$ and $\Delta^{em}(e_n, e_{a,2}, a)$ in Figure~\ref{fig:relation-pair-avg-r-vs-unembedding-cosine}, these correlate even less.

    \begin{figure}[h]
        \centering
        \includegraphics[width=\columnwidth]{figures/animal_pair_avg_r_vs_unembedding_cosine.png}
        \caption{
            Pairwise correlation between $\Delta^{em}(e_n, e_a, a_1)$ and $\Delta^{em}(e_n, e_a, a_2)$ plotted against the cosine similarity of the corresponding unembedding vectors.
    ...
    \begin{figure}[h]
        \centering
        \includegraphics[width=\columnwidth]{figures/relation_pair_avg_r_vs_unembedding_cosine.png}
        \caption{
            Pairwise correlation between $\Delta^{em}(e_n, e_{a,1}, a)$ and $\Delta^{em}(e_n, e_{a,2}, a)$ plotted against the cosine similarity of the corresponding unembedding vectors.
...
\end{qualitymark}
    ...
    \begin{figure}[h]
        \centering
        \includegraphics[width=\columnwidth]{figures/heatmap_corr_by_relation_single_token.png}
        \caption{Pairwise correlation between $\Delta^{em}(e_n, e_a, a_1)$ and $\Delta^{em}(e_n, e_a, a_2)$ for single-token animals.
        $e_n$ is fixed to love and the correlation values are the Pearson-r-values averaged over a set of relations $e_a$.
        Higher values indicate that two animals are influenced similarly by the same numbers.}\label{fig:heatmap_corr_by_relation_single_token}
    \end{figure}
\end{document}